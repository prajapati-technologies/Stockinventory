...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="stats-card">
            ...
            <form method="POST" action="{{ route('store.customer.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row g-3">
                        ...
                        >
                            ...
                            @foreach(App\Models\District::where('is_active', true)->get() as $district)
                                <option value="{{ $district->id }}" 
                                    {{ (old('district_id', $store->district_id) == $district->id) ? 'selected' : '' }}>
                                    {{ $district->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        >
                            ...
                            @if($store->mandal_id)
                                <option value="{{ $store->mandal_id }}" selected>{{ $store->mandal->name }}</option>
                            @endif
                        </select>
                        @error('mandal_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-12">
                        <label class="form-label">Document Photo (Optional - Max 100KB)</label>
                        <input 
                            type="file" 
                            name="document_photo" 
                            accept="image/*" 
                        ...
                        @error('document_photo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Image will be automatically compressed to fit size requirements</div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    ...
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save me-2"></i>Save & Proceed to Sale
                    </button>
                </div>
            </form>
        </div>
...
// Load mandals on page load if district is already selected
document.addEventListener('DOMContentLoaded', function() {
    const districtSelect = document.getElementById('district_id');
    if (districtSelect.value) {
        loadMandals(districtSelect.value);
    }
});
        ...
        .then(data => {
            if (data.exists) {
                statusDiv.innerHTML = `
                    ...
                    <a href="/store/customer/search?document_number=${documentNumber}" class="btn btn-info btn-sm">
